...
- MultiScaleSampler: variable-resolution batch sampler used by the multiscale
  loader.
- build_fixed_scale_loader: DataLoader with a constant batch size at a single
  fixed spatial resolution.
- build_multi_scale_loader: DataLoader with a variable batch size and a
  spatial resolution that is resampled every step.

The two loader factories correspond to the two batch-size strategies in the
MobileViT paper (Mehta and Rastegari, ICLR 2022, Section 3.2):
1. Fixed scale: constant batch size at one resolution. Used for evaluation
   and for fixed-scale training.
2. Multi-scale: the resolution is sampled per batch from a configured set
   and the batch size is rescaled so batch * height * width stays roughly
   constant. Used for training only.

Batch-size scaling formula (multiscale)
----------------------------------------
Given a base resolution s_0 and base batch size b_0, the batch size at
resolution s is:

    b(s) = max(1, round(b_0 * s_0^2 / s^2))

Smaller resolutions yield larger batches, larger resolutions yield smaller
ones, so the total pixel count per step is approximately constant.

Both factories accept a dataset whose __getitem__ returns (PIL image, label).
The transform is applied inside a thin dataset wrapper, so the underlying
dataset object stays untouched and can be reused at any resolution.

Only models that accept arbitrary input resolutions should be trained with the
multi-scale loader. MobileViT and the torchvision CNNs are safe; EfficientViT
and EfficientFormer are fixed-resolution (see
``models.registry.supports_dynamic_input``). The training script checks this
flag before enabling multi-scale sampling.
...
... )
>>>
>>> train_ds, val_ds = build_imagenet100k_datasets(root="./imagenet100")
>>>
...
# Type aliases:
# - TransformFn maps a PIL image to a normalized tensor.
# - TransformBuilder takes a spatial size and returns such a transform.
...
class MultiScaleSampler(Sampler[list[tuple[int, int, int]]]):
    ...
    fetches the underlying image, and applies the training transform at
    ...
    Determinism
    -----------
    The full schedule of batches for an epoch is precomputed at construction
    time and rebuilt whenever ``set_epoch`` is called. The schedule depends
    only on (seed, epoch), so the same epoch always produces the same
    sequence of batches.
    ...
    base_size:
        Reference resolution s_0 used to anchor the batch-size scaling.

    base_batch_size:
        Batch size b_0 used when the sampled resolution equals base_size.
    ...
    def __init__(
        ...
        # Precompute batch sizes for each scale using the formula above.
    ...
    def _build_schedule(self, epoch: int) -> list[list[tuple[int, int, int]]]:
        ...
        The resolution of each batch is drawn from ``self.scales``, and the
        batch size is the corresponding entry in ``self.batch_size_per_scale``.
        Two independent RNGs (seeded from ``seed + epoch``) are used so
        shuffling and scale sampling are decoupled.
        """
        ...
        while position < len(indices):
            ...
            # Trailing partial batch: skip if drop_last is True.
...
class _FixedScaleDataset(Dataset):
    ...
    The transform is applied lazily in __getitem__ so the underlying
    base dataset is untouched. Returns (tensor, label).
    """
...
class _MultiScaleDataset(Dataset):
    ...
    __getitem__ accepts a (height, width, base_index) tuple produced by the
    sampler. The training transform is built lazily for each (height, width)
    pair and cached, so we only pay the construction cost once per resolution
    per worker.
    """
...
def build_fixed_scale_loader(
    ...
    return DataLoader(
        ...
        drop_last=drop_last,
        # Keep workers alive across epochs; large speedup on Windows where
...
def build_multi_scale_loader(
    ...
    Every batch is produced at a spatial resolution sampled from scales.
    The batch size at each resolution is chosen so that
    batch * size**2 stays approximately constant, anchored to
    ``base_batch_size`` at ``base_size``.
    ...
    Notes
    -----
    Call ``loader.batch_sampler.set_epoch(epoch)`` at the start of every
    epoch to advance the sampler's RNG deterministically. The provided
    ``Trainer`` already does this automatically.
    """
    if num_workers < 0:
        raise ValueError("num_workers must be a non-negative integer.")
